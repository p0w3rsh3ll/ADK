...
Process {
    ...
    $adkGenericURL = (Invoke-WebRequest -Uri 'https://go.microsoft.com/fwlink/?linkid=2290318' -UseBasicParsing -MaximumRedirection 0 -ErrorAction SilentlyContinue)

    # 302 = redirect as moved temporarily
    # 301 = Moved Permanently
    if ($adkGenericURL.StatusCode -eq 302) {

        # Currently set to https://download.microsoft.com/download/5/5/6/556e01ec-9d78-417d-b1e1-d83a2eff20bc/ADKWinPEAddons/
        # Resolving download root for: https://go.microsoft.com/fwlink/?linkid=2290318
...
15=Windows PE Boot Files (DesktopEditions)-x86_en-us.msi
16=Windows PE Boot Files (OnecoreUAP)-x86_en-us.msi
17=Windows PE Optional Packages (DesktopEditions)-x86_en-us.msi
18=Windows PE Scripts-x86_en-us.msi
19=Windows PE wims (DesktopEditions)-x86_en-us.msi
'@
        }
        ...
        try {
            ...
            Invoke-WebRequest -Uri "$($MainURL)adkwinpesetup.exe" -UseBasicParsing -OutFile "$($TargetFolder)\adkwinpesetup.exe" @HT
